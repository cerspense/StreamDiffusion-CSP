...
**Project:** StreamDiffusion + TouchDesigner Integration (dotsimulate fork)
**Focus:** Building Dynamic Pipeline Processors (Fx) for Real-Time Image Generation
**Branch:** `SDTD_v3_experimental`
**Date:** 2025-10-30
...
This project implements a **plugin-based custom processor system** with **explicit stage metadata** that enables real-time control of latent-domain and image-domain processors through TouchDesigner. The system automatically discovers processors, generates TouchDesigner parameters from metadata, supports live OSC updates, and seamlessly integrates with the StreamDiffusion pipeline.

**Key Architecture:**
- **Plugin-Based Discovery:** Drop processors into `custom_processors/` folder - auto-discovered on startup
- **4-Stage Hook System:** Explicit stage metadata (`image_pre`, `latent_pre`, `latent_post`, `image_post`)
- **Metadata-Driven:** Single source of truth in `get_preprocessor_metadata()` (now includes `stage` field)
- **TouchDesigner Integration:** Auto-generates UI parameters, OSC handlers, and YAML config
- **Zero-Code Extension:** Create processor file with metadata - no manual registration needed!

---

## 🚀 What's New in v2.0 (v3_experimental branch)

**MAJOR CHANGES - Read This First!**

1. **Plugin-Based System:** Processors now live in `custom_processors/` folder - no manual registration!
2. **Stage Metadata Required:** All processors MUST declare `"stage"` in metadata (e.g., `"stage": "latent_pre"`)
3. **Absolute Imports Only:** Use `from streamdiffusion.preprocessing.processors import ...` (not `from .base import ...`)
4. **Auto-Discovery:** Just drop a `.py` file in `custom_processors/your_collection/` and pulse refresh - done!
5. **90% Faster Workflow:** 6 minutes vs 62 minutes for old manual system

**Migration Checklist (for existing processors):**
```
[ ] Move processor files to custom_processors/your_collection/
[ ] Add "stage" field to get_preprocessor_metadata()
[ ] Change relative imports to absolute imports
[ ] Add processors.yaml manifest (optional but recommended)
[ ] Pulse Refreshfxmetadata in TouchDesigner
[ ] Verify processor appears in table_preprocessors with correct stage
```

**See:** [Development Workflow](#development-workflow) for step-by-step guide.
...
**CRITICAL:** All processors must declare their pipeline stage explicitly via the `"stage"` field in metadata:

```python
@classmethod
def get_preprocessor_metadata(cls):
    return {
        "stage": "latent_pre",  # ← REQUIRED: "image_pre", "latent_pre", "latent_post", or "image_post"
...
**Stage Values:**
- `"image_pre"` - Image preprocessing (before VAE encode)
- `"latent_pre"` - Latent preprocessing (after VAE encode, before diffusion)
- `"latent_post"` - Latent postprocessing (after diffusion, before VAE decode)
- `"image_post"` - Image postprocessing (after VAE decode)

**Backwards Compatibility:** Processors without stage metadata default to `"image_pre"`, but this is deprecated - always specify explicitly!

---

### 1. Image Preprocessing - `"image_pre"` (Before VAE Encode)

**Stage Value:** `"image_pre"`
**Location:** `stream.image_preprocessing_hooks`
**File:** `custom_processors/your_collection/your_processor.py`
**Use Cases:**
...
### 2. Latent Preprocessing - `"latent_pre"` (After VAE Encode, Before Diffusion) ⭐

**Stage Value:** `"latent_pre"`
...
### 3. Latent Postprocessing - `"latent_post"` (After Diffusion, Before VAE Decode) 🔮

**Stage Value:** `"latent_post"`
...
### 4. Image Postprocessing - `"image_post"` (After VAE Decode)

**Stage Value:** `"image_post"`
...
### Quick Start: Plugin-Based Workflow

**NEW!** Processors are now created as **plugins** in the `custom_processors/` folder. No manual registration needed!

**Steps:**
1. Create a `.py` file in `custom_processors/your_collection/`
2. Implement processor class with metadata (including `stage` field)
3. Refresh metadata in TouchDesigner
4. Done! Processor auto-discovered and integrated
...
```python
# CRITICAL: Use ABSOLUTE imports (not relative imports!)
from streamdiffusion.preprocessing.processors import BasePreprocessor, PipelineAwareProcessor

# Use PipelineAwareProcessor if you need access to previous frame data
# Use BasePreprocessor for stateless processing
```

**IMPORTANT:** Custom processors MUST use absolute imports. Relative imports (e.g., `from .base import`) will fail in the plugin system.
...
def get_preprocessor_metadata(cls):
    return {
        "stage": "latent_pre",  # ← NEW! REQUIRED: Declare pipeline stage explicitly
...
- `"image_pre"` - Before VAE encode (RGB domain)
- `"latent_pre"` - After VAE encode, before diffusion (latent domain)
- `"latent_post"` - After diffusion, before VAE decode (latent domain)
- `"image_post"` - After VAE decode (RGB domain)
...
### Step 5: Create Processor Collection (Optional)

**NEW!** Processors are organized into collections in the `custom_processors/` folder.

**Folder Structure:**
```
custom_processors/
├── README.md
└── my_collection/              # Your processor collection
    ├── processors.yaml         # Optional manifest
    ├── my_processor.py
    └── another_processor.py
```

**Option A: With Manifest (Recommended)**

Create `processors.yaml` to control load order:
```yaml
processors:
  - my_processor.py
  - another_processor.py
...
If no `processors.yaml` exists, all `.py` files are auto-discovered (except `_*.py`, `base.py`, `setup.py`).

---

### Step 6: Auto-Discovery & Registration

**No manual registration needed!** The plugin system automatically:

1. **Scans `custom_processors/` folder** on StreamDiffusion startup
2. **Loads processor files** listed in `processors.yaml` (or auto-discovers `.py` files)
3. **Registers processors** in the global `_preprocessor_registry`
4. **Validates stage metadata** to ensure processors are assigned to correct pipeline stage

**Check Discovery Logs:**
```
INFO: Scanning custom_processors/ folder for custom processors...
INFO: Loading processor collection 'my_collection' (2 processors)
INFO: Registered custom processor: my_processor (MyProcessorPreprocessor)
```

**Name Collision Protection:** Custom processors with names matching core processors are rejected automatically.

---

### Step 7: Refresh Metadata in TouchDesigner

After adding a new processor file, refresh the metadata table:

1. **Pulse the `Refreshfxmetadata` button** on the Fx page
2. Fx* parameters appear automatically!

**What this does:**
- Calls `Getpreprocessors()` via subprocess to import runtime registry
- Extracts metadata from all registered processors (including custom ones)
- Auto-generates Fx* parameters based on metadata
- Updates `table_preprocessors` DAT with stage information
...
**That's it!** No editing `__init__.py`, no editing `StreamDiffusionExt.py` - the system handles everything automatically.
...
**NEW! Time Estimate:** 5-10 minutes per processor (down from 10-15 min with old workflow!)

### Quick Reference Checklist

```
[ ] 1. Create processor .py file in custom_processors/your_collection/
[ ] 2. Implement class with get_preprocessor_metadata() (including "stage" field!)
[ ] 3. Pulse Refreshfxmetadata in TouchDesigner
[ ] 4. Verify Fx* parameters appear
[ ] 5. Test live OSC updates
```

**That's it!** No manual registration, no editing core files.

---

### Step 1: Create Processor Class (3 min)
**File:** `custom_processors/my_collection/my_processor.py`
...
class MyProcessor(BasePreprocessor):
    ...
    def get_preprocessor_metadata(cls):
        ...
        return {
            "stage": "image_post",  # ← NEW! REQUIRED field
            "display_name": "My Processor",
...
### Step 2: Optional - Add to Manifest (1 min)

If your collection has a `processors.yaml` manifest, add your processor:

**File:** `custom_processors/my_collection/processors.yaml`

```yaml
processors:
  - my_processor.py  # ← Add this line
  - existing_processor.py
```

If no manifest exists, your processor will be auto-discovered - skip this step!

---

### Step 3: Refresh and Test (1 min)

**In TouchDesigner:**

1. **Pulse `Refreshfxmetadata` button** on the Fx page
2. Fx* parameters appear automatically!
3. Verify processor appears in `table_preprocessors` DAT with correct stage

**That's it!** No manual registration, no editing core files.

---

### Step 4: Test and Verify (1 min)
...
**Common Issues:**
- ❌ Fx* params don't appear → Check discovery logs for import errors
- ❌ Processor doesn't load → Verify `stage` field in metadata, check logs
- ❌ OSC updates don't work → Check processor name matches file name (minus `.py`)
- ❌ Import errors → Use absolute imports (`from streamdiffusion.preprocessing.processors import ...`)
...
### File Locations
- **Custom Processors:** `custom_processors/your_collection/your_processor.py` ← NEW!
- **Core Processors:** `src/streamdiffusion/preprocessing/processors/` (read-only reference)
- **Discovery System:** `src/streamdiffusion/preprocessing/processors/__init__.py` (auto-runs on import)
...
- **Parameter Generation:** Auto-generated from metadata (no manual editing!)
- **Refresh Button:** Pulse `Refreshfxmetadata` to reload processor registry

### Metadata Example
```python
{
    "stage": "latent_pre",  # ← NEW! REQUIRED: Declare pipeline stage
    "display_name": "My Processor",
    "description": "What it does",
    "parameters": {
        "param_name": {
...
**v3_experimental (NEW Plugin System):**

| Step | Old Manual System | New Plugin System | Time Savings |
|------|-------------------|-------------------|--------------|
| Create processor class | 10 min | 3 min | Auto-discovery template |
| Register in __init__.py | 5 min | **0 min** ✅ | Auto-registered! |
| TD Extension wiring | 15 min | **0 min** ✅ | Auto-generated! |
| YAML generation | 20 min | **0 min** ✅ | Auto-generated! |
| Add to manifest (optional) | N/A | 1 min | Simple YAML entry |
| TD refresh | 2 min | 1 min | One-click refresh |
| Testing | 10 min | 1 min | Faster iteration |
| **TOTAL** | **62 min** | **6 min** | **90% faster!** |

**Key Improvements:**
- ✅ No more manual registration in `__init__.py`
- ✅ No more editing `StreamDiffusionExt.py` for toggles
- ✅ No more YAML generation code
- ✅ Just add `stage` field to metadata and drop file in folder
- ✅ Refresh button → instant integration

---

### Common Gotchas and Solutions (v3_experimental)

**Gotcha #1: "Processor not discovered" / "No logs about my processor"**
- **Cause:** Import errors (relative imports fail in plugin system)
- **Solution:** Use absolute imports: `from streamdiffusion.preprocessing.processors import BasePreprocessor`
- **Test:** Check logs for `ERROR - Failed to load my_processor.py: ...`

**Gotcha #2: "Name conflict: processor 'X' already exists in registry"**
- **Cause:** Custom processor name matches a core processor name
- **Solution:** Rename your processor file to something unique
- **Test:** Check `list_preprocessors()` output for duplicates

**Gotcha #3: "Processor loads but appears in wrong pipeline stage"**
- **Cause:** Missing or incorrect `"stage"` field in metadata
- **Solution:** Add `"stage": "latent_pre"` (or appropriate value) to `get_preprocessor_metadata()`
- **Test:** Check `table_preprocessors` DAT - stage column should show correct value

**Gotcha #4: "Fx* parameters don't appear after refresh"**
- **Cause:** Discovery failed, metadata format invalid, or class not inheriting from base
- **Solution:** Check logs, verify metadata dict format, verify class inherits from `BasePreprocessor`
- **Test:** Run `python -c "from custom_processors.my_collection.my_processor import MyProcessor; print(MyProcessor.get_preprocessor_metadata())"` to test import

**Gotcha #5: "Image postprocessor creates 1-frame delay"**
...
**Gotcha #6: "Processor works locally but breaks in production"**
- **Cause:** Custom processors auto-discovery enabled in production
- **Solution:** Set `STREAMDIFFUSION_DISABLE_CUSTOM_PROCESSORS=1` env var
- **Test:** Check logs for "Custom processor discovery disabled"
...
**Branch:** `SDTD_v3_experimental` on https://github.com/dotsimulate/StreamDiffusion
**Maintainer:** Spencer (CSP) + Lyell (dotsimulate)
**Related Documents:**
- `custom_processors/README.md` - Plugin system documentation
- `custom_processor_stage_system_handoff.md` - Developer handoff from Lyell
- `multistage_processing_system_architecture.md`
- `fx_dynamic_pipeline_processors_implementation_2025-10-23.md`

**Changelog:**
- **v2.0 (2025-10-30):** 🚀 MAJOR UPDATE - Plugin-based custom processor system
  - **NEW:** Auto-discovery from `custom_processors/` folder (no manual registration!)
  - **NEW:** Explicit stage metadata system (`"stage"` field required in metadata)
  - **CHANGED:** Absolute imports required (not relative imports)
  - **CHANGED:** Workflow reduced from 62 min → 6 min (90% faster!)
  - **CHANGED:** No more editing `__init__.py` or `StreamDiffusionExt.py`
  - **ADDED:** Manifest system (`processors.yaml`) for load order control
  - **ADDED:** Name collision protection for core processors
  - **ADDED:** Environment variable to disable discovery in production
