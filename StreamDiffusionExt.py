...
"""
        ...
        use_color_correction = (hasattr(self.ownerComp.par, 'Usecolorcorrection') and
                               self.ownerComp.par.Usecolorcorrection.eval())
...
"""
            ...
            if use_color_correction:
                params = self.gather_fx_parameters_for_processor('color_correction')
                yaml_content += f'    - type: "color_correction"\n'
                yaml_content += f'      order: {processor_order}\n'
                yaml_content += f'      enabled: true\n'
                yaml_content += f'      params:\n'
                for param_name, param_value in params.items():
...
"""
    ...
    def Usecolorcorrection(self):
        self.logger.log('Usecolorcorrection changed', level='INFO')
        """Called when Usecolorcorrection toggle changes - updates Fx dynamic parameters"""
        self.update_fx_dynamic_parameters()
    ...
    def update_fx_dynamic_parameters(self):
        if not self.ownerComp.par.Updatefxpars.eval():
            return
        """Updates Fx* parameters for feedback_transform (image), latent_feedback, latent_transform, and color_correction on Fx page"""
        active_fx = []
        ...
        if hasattr(self.ownerComp.par, 'Usecolorcorrection') and self.ownerComp.par.Usecolorcorrection.eval():
            active_fx.append('color_correction')
