...
class PreprocessSharpenPreprocessor(BasePreprocessor):
    ...
    def _process_tensor_core(self, image_tensor: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            image_tensor: [B, C, H, W] or [C, H, W] in range [-1, 1] (from VaeImageProcessor)

        Returns:
            Processed tensor, same shape, in range [-1, 1]
        """
        ...
        # CRITICAL FIX: Normalize input from [-1, 1] to [0, 1] for processing
        ...
        # Clamp to [0, 1] to prevent color space drift
