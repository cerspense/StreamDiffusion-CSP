...
class FeedbackTransformPreprocessor(PipelineAwareProcessor):
    ...
    Combines feedback loop with geometric transforms (zoom, pan, rotate) applied to previous output image.
    Operates in image space BEFORE VAE encoding, providing Deforum-style motion with temporal consistency.
    ...
    Examples:
    - zoom=1.05, feedback_strength=0.8: 5% zoom per frame with 80% feedback
    - pan_x=0.01, feedback_strength=0.5: Panning with 50/50 input/feedback blend
    - rotation=2.0, feedback_strength=1.0: Pure feedback rotation loop
    ...
    def get_preprocessor_metadata(cls):
        return {
            ...
            "description": "Image-space feedback with geometric transformations for Deforum-style effects",
            "parameters": {
                "feedback_strength": {
                    "type": "float",
                    "default": 0.8,
            ...
            "use_cases": [
                "Deforum-style camera motion in image space",
