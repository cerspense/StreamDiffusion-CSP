...
class LatentTransformCCPreprocessor(PipelineAwareProcessor):
    """
    Latent-space transformation processor with color correction and noise injection

    Extends LatentTransformPreprocessor with color correction capabilities to combat
    greying/color loss in feedback loops.

    Key Features:
    - Zoom: Scale latent tensor (interpolate + crop/pad)
    - Pan: Translate latent tensor in X/Y
    - Rotate: Rotate latent tensor around center
    - Brightness: Adjust overall luminance in latent space
    - Contrast: Control dynamic range
    - Black Level: Lift/crush shadows
    - Saturation: Control color intensity (approximate in latent space)
    ...
    Use this variant when experiencing color drift or greying in feedback loops.
    The color correction happens in latent space, which is less precise than RGB
    adjustments but maintains efficiency by avoiding VAE decode/encode cycles.

    Examples:
    - saturation=1.15, black_level=-0.08: Combat greying in feedback loops
    - contrast=1.1, brightness=-0.02: Maintain dynamic range and prevent washout
    - zoom=1.05, saturation=1.2: Zooming with color boost
    """

    @classmethod
    def get_preprocessor_metadata(cls):
        return {
            "display_name": "Latent Transform CC (Zoom/Pan/Rotate + Color)",
            "description": "Geometric transformations with color correction in latent space - prevents greying in feedback loops",
            "parameters": {
                "zoom": {
                    "type": "float",
                    "default": 1.0,
                ...
                "brightness": {
                    ...
                    "range": [-0.5, 0.5],
                    "step": 0.01,
                    "description": "Brightness adjustment in latent space (positive = brighter, negative = darker)"
                },
                "contrast": {
                    "type": "float",
                    "default": 1.0,
                    "range": [0.5, 2.0],
                    "step": 0.01,
                    "description": "Contrast adjustment in latent space (1.0 = no change, >1.0 = more contrast, <1.0 = less contrast)"
                },
                "black_level": {
                    "type": "float",
                    "default": 0.0,
                    "range": [-0.3, 0.3],
                    "step": 0.01,
                    "description": "Black level adjustment - lifts/crushes shadows in latent space"
                },
                "saturation": {
                    "type": "float",
                    "default": 1.0,
                    "range": [0.0, 2.0],
                    "step": 0.01,
                    "description": "Color saturation in latent space (1.0 = normal, >1.0 = more saturated, <1.0 = desaturated, 0.0 = greyscale)"
                },
                "noise_strength": {
                    "type": "float",
                    "default": 0.0,
            ...
            "use_cases": [
                "Deforum-style camera motion with color control",
                "Feedback loops with anti-greying",
                "Latent space animation with saturation boost",
                "Zoom/pan with brightness/contrast adjustment",
                "Temporal consistency with color preservation"
            ]
        }
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize latent transform CC preprocessor
        ...
        Args:
            ...
            brightness: Brightness adjustment in latent space (-0.5 to 0.5)
            contrast: Contrast adjustment in latent space (0.5 to 2.0, 1.0 = no change)
            black_level: Black level adjustment - lifts/crushes shadows (-0.3 to 0.3)
            saturation: Color saturation in latent space (0.0 to 2.0, 1.0 = normal)
    ...
    def _process_core(self, image):
        ...
        raise NotImplementedError(
            "LatentTransformCCPreprocessor is designed for latent domain processing. "
    ...
    def _apply_color_adjustments(self, latent: torch.Tensor) -> torch.Tensor:
        """
        Apply brightness, contrast, black level, and saturation adjustments in latent space

        This method manipulates the latent tensor to affect the decoded image's color properties.
        While latent space doesn't directly map to RGB, these transformations will influence
        the final image's brightness and color characteristics.

        Approach:
        - Brightness: Direct offset to all channels (affects overall luminance)
        - Contrast: Scale around mean (affects dynamic range)
        - Black Level: Additive offset that lifts/crushes shadows
        - Saturation: Channel variance manipulation (approximate color intensity)

        Args:
            latent: Latent tensor [B, C, H, W]

        Returns:
            Color-adjusted latent tensor
        """
        # Check if any adjustment is needed
        needs_adjustment = (
            abs(self.brightness) > 1e-6 or
            abs(self.contrast - 1.0) > 1e-6 or
            abs(self.black_level) > 1e-6 or
            abs(self.saturation - 1.0) > 1e-6
        )
        ...
        # ======================================================================
        # 1. BLACK LEVEL ADJUSTMENT (Lift/Crush)
        # ======================================================================
        # Add a bias before other operations to lift or crush the blacks
        if abs(self.black_level) > 1e-6:
            result = result + self.black_level

        # ======================================================================
        # 2. CONTRAST ADJUSTMENT
        # ======================================================================
        # Scale around the per-channel mean
        if abs(self.contrast - 1.0) > 1e-6:
            # Calculate mean per channel across spatial dimensions
            # Shape: [B, C, H, W] -> [B, C, 1, 1]
            channel_mean = result.mean(dim=[2, 3], keepdim=True)

            # Scale deviation from mean
            result = channel_mean + self.contrast * (result - channel_mean)
        ...
        # ======================================================================
        # 4. SATURATION ADJUSTMENT (Approximate in Latent Space)
        # ======================================================================
        # In latent space, we approximate saturation by manipulating channel variance
        # Higher saturation = stronger channel differentiation
        # Lower saturation = channels converge toward their mean
        if abs(self.saturation - 1.0) > 1e-6:
            # Calculate global mean across all channels
            # This approximates "greyscale" in latent space
            global_mean = result.mean(dim=1, keepdim=True)  # [B, 1, H, W]

            # Blend between global mean (desaturated) and original channels (saturated)
            # saturation=0.0 -> all channels = mean (grey)
            # saturation=1.0 -> original (normal color)
            # saturation>1.0 -> exaggerated channel differences (more saturated)
            result = global_mean + self.saturation * (result - global_mean)
    ...
    def _process_tensor_core(self, tensor: torch.Tensor) -> torch.Tensor:
        """
        Process latent tensor with geometric transformations, color adjustments, and noise injection

        Processing Pipeline (DEFORUM-STYLE + COLOR CORRECTION):
        ...
        4. Apply color adjustments (brightness, contrast, black level, saturation)
        5. Apply noise injection (if noise_strength > 0)
        6. Safety clamp to prevent extreme values

        Args:
            tensor: Current input latent tensor [B, C, H/8, W/8]

        Returns:
            Transformed, color-adjusted, re-noised, and optionally blended latent tensor
        """
        ...
        # ======================================================================
        # STEP 4: Apply Color Adjustments (Brightness, Contrast, Black Level, Saturation)
        # ======================================================================

        result_latent = self._apply_color_adjustments(result_latent)

        # ======================================================================
        # STEP 5: Apply Noise Injection (if enabled)
        # ======================================================================
